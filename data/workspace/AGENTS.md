...
- **delivery.channel**: isolated session では `"last"` は使わない。明示的に `"webchat"` を指定
- **timeout**: 処理内容に応じて 180-300 秒を設定。重い処理はサブエージェント分割を検討
- **エラー記録**: delivery エラーでもタスクが完了している場合がある。ログを確認して判断
